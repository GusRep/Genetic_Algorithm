% Cortador de caños mediante Algoritmos Genéticos: (rev. 20-04-08)
% ================================================
...
% enteros, el primero es la cantidad de caños cortados necesarios de la 
...
% elementos sin usar (por ej. que el numero 13 de codificación binaria 
% nunca apareció, mientas que otras codificaciones aparecieron repetidas 
...
% número tal (contenido de Datos[i][n]), de long Largos[i].
% 
...
% bajo sea el valor numérico de nuestra función, menos cantidad de tiras 
...
% c) Desp = 0
% situacion óptima, mayor aprovechamiento del liston (premiamos) x5 
DespC=sum(MaterialRestantePorTira==0);                         %MAXIMIZAR

% Obtenemos la Sumatoria de desperdicios, recordar que menor valor numérico
...
AptiProm=(sum(Aptitud))/P;           %aptitud promedio de esta la población
...
%                 - Mutación (anomalía genética en la copia de genes a la 
%                   siguiente generación de individuos)
%               
% Ahora definiremos en base a la "aptitud" que posee cada individuo de la 
% población, con respecto a la aptitud promedio que posee dicha población,
% quienes son las mejores soluciones (valor numérico de la aptitud baja) y 
% quienes son las peores soluciones.
% Las mejores soluciones se Aparearan, para intercambiar material genético 
% para intentar obtener una mejor solución, las peores soluciones tendrán 
% la posibilidad de mutar para ver si se pueden encaminar hacia una mejor 
% solución, mientras tanto la media de las soluciones pasara directamente 
% a la siguiente generación.

% El umbral de apareo debe ser menor que la media, ya que menor aptitud es
% mejor. Estoy empleando un criterio de selección muy simple: EL ELITISTA.
if IteraActual<=(CantItera/2)
    UmbralApareo=0.97*AptiProm;     
else
    UmbralApareo=0.98*AptiProm;
end
% Notar que soy mas ELITISTA en la selección de individuos, al comienzo de 
% la evolución de mi población.
...
% El apareamiento para realizar el intercambio genético, se producira a 
% través de una mascara binaria generada de manera aleatoria, de esta 
% manera podemos independizarnos la posición en la que se encuentran los 
% genes, ya que si tenemos una secuencia de genes 1-2-3-4-5-6-7-8, podemos
% hacer que sólo se intercambien los genes 1 y 3, sin necesidad de 
% intercambiar el gen 2 que se encuentra entre éstos.
...
if a2>1
    ...
    %población (valor numérico mas grande), y guardaremos en 
...
if (1==ProbaMuta)       
    % con ROUND redondeo al entero mas cercano y con RANDN obtengo números 
    % entre -1 y 1
    Datos(randsrc(1,1,[1:256]),randsrc(1,1,[1:P]))=randsrc(1,1,[1:256]);
        m=1;                  %solo una mutación
end
...
%Gráficas de los indicadores de funcionamiento del algoritmo:
%------------------------------------------------------------
...
j=min(Aptitud);
i=find(Aptitud==j);                  
%indice de la mejor solución de la ultima población(menor aptitud numérica)
