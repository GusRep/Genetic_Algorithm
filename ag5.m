% Cortador de caños mediante Algoritmos Genéticos: (rev. 07-01-08)
% ================================================
...
for iteraciones=1:10             %======== INICIO DEL PRE PROCESANIENTO ==========   
...
for j=1:P
    for i=1:CantDeTirasTotales
        if Datos(i,j)>0                                         
            % Datos(i,j)=Número de tira a la que pertenece el corte de Largos[i]
            Valor(Datos(i,j),j)=Valor(Datos(i,j),j)+Largos(i);          %sumatoria // Fila:Datos(i,j) Columna:j         
        end
    end
end

% Aca en Valor debemos tener ceros en donde no se usaron determinados
% listones, los ceros deben coincidir con la matriz de Datos.
...
Aptitud=Lmax*CantidadDeTirasEmpleadas+Desp+(LargosTot-SumValor).^4;       %aptitud del conjunto de soluciones
...
% Generamos la matriz "Datos" de [CantDeTirasTotales x P]
Datos=randint(CantDeTirasTotales,P,[0,CantDeTirasEstimada]);     %generamos la poblacion inicial de soluciones, de manera RANDOM (Aleatoria)
Datos=Datos.*randsrc(CantDeTirasTotales,P,[0,1;0.2,0.8]);      %agregamos ceros
...
Aptitud=Lmax*CantidadDeTirasEmpleadas+Desp+Coherencia;       %aptitud del conjunto de soluciones
AptiProm=(sum(Aptitud))/P;                              %aptitud promedio de esta la poblacion
