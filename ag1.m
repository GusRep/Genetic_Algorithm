...
% Generamos la matriz "Datos" de [2*CantDeTirasTotales x P]

Datos=randsrc(2*CantDeTirasTotales,P,[0:1:CantDeTirasTotales]);     %generamos la poblacion inicial de soluciones, de manera RANDOM (Aleatoria)
Ceros=randsrc(2*CantDeTirasTotales,P,[0 1]);                               %  agregamos ceros en forma Random en la poblacion
Datos=Datos.*Ceros;
...
Valor=zeros(2*CantDeTirasTotales,P);       %cada posicion sera el numero de tira al que pertenece el corte (total de tiras = CantDeTirasTotales)
...
MaterialRestantePorTira=ones(2*CantDeTirasTotales,P)*Lmax.*(~TirasNoEmpleadas);
...
Desp=1*DespA+100*abs(DespB)-10*DespC;
...
UmbralApareo=0.96*AptiProm;              %el umbral de apareo debe ser menor que la media, ya que menor aptitud es mejor
...
Mask=randsrc(2*CantDeTirasTotales,1,[0:1]);              %generamos un vector mascara binaria, de manera RANDOM
...
if a2>1
    ...
    %los peores individuos de la poblacion, y guardaremos en "Indice_Para_Reemplazar" el indice que se corresponde
    %en la matriz "Datos"
