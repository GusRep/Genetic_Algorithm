% Cortador de caños mediante Algoritmos Genéticos: (rev. 12-03-08)
% ================================================

% Importaremos un archivo de texto con los datos separados por " ; " que en su primer fila contiene la longitud 
...
% INICIO DEL PRE PROSESAMIENTO IMPONIENDO LA PEOR
% CONDICION PARA LA CANTIDAD DE TIRAS A EMPLEAR (CantDeTirasTotales)
CantDeTirasAEmplear=CantDeTirasTotales;
Datos=randint(CantDeTirasAEmplear,P,[0,CantDeTirasTotales]);     %generamos la poblacion inicial de soluciones, de manera RANDOM (Aleatoria)
Datos=Datos.*randsrc(CantDeTirasAEmplear,P,[0,1;0.2,0.8]);      %agregamos ceros

% Datos=Datos([1:CantDeTirasTotales],[1:P]);  %PARCHE

% ANALIZANDO EL PROGRAMA:
% Por el momento veremos que en la matriz "Datos" nos quedaron varios elementos en cero, esto significa que
% esa codificacion binaria de tira no se empleo en la solucion. Las que son distintas de cero, como vimos
% es lo que se aprovecho del liston número tal (contenido de Datos[i][n]), de long Largos[i].
% 
...
% La matriz "Valor" contiene la sumatoria de los largos de caño empleado
% por las soluciones.

% Ahora calcularemos el desperdicio que resulta de cada liston utilizado haciendo:
% 
...
% INICIO DEL PRE PROSESAMIENTO CON 10 GENERACIONES IMPONIENDO LA PEOR
% CONDICION PARA LA CANTIDAD DE TIRAS A EMPLEAR

for iteraciones=0%1:1             %======== INICIO DEL PRE PROCESANIENTO ==========   
...
% Datos=Datos([1:CantDeTirasAEmplear],[1:P]);  %PARCHE
for j=1:P
    %obtenemos la cantidad de veces que se repite cierta codificacion y la
    %multiplicamos por su largo
    for i=1:CantDeTirasAEmplear
        cant=0;                     
        for k=1:CantDeTirasAEmplear
            if (Datos(k,j))==i
                cant=cant+1;
            end
        end
        Valor(i,j)=cant*Largos(i);
    end
end
...
%Vector de cantidad de tiras empleadas en cada solucion
...
% Obtenemos la Sumatoria de desperdicios, recordar que menor valor numerico
% es mas Apto !!!      -->     MINIMIZAR
Desp=DespA+abs(DespB)+(CantDeTirasTotales-DespC);

% Contemplamos que la long de nuestra solucion sea la necesaria
SumValor=sum(Valor);

Aptitud=CantidadDeTirasEmpleadas+Desp;       %aptitud del conjunto de soluciones
AptiProm=(sum(Aptitud))/P;                              %aptitud promedio de esta la poblacion

 
UmbralApareo=0.95*AptiProm;              %el umbral de apareo debe ser menor que la media, ya que menor aptitud es mejor
...
Mask=randsrc(CantDeTirasAEmplear,1,[0:1]);              %generamos un vector mascara binaria, de manera RANDOM
...
end     %======== FIN DEL PRE PROCESAMIENTO ==========

% OBTENEMOS UNA CANT DE TIRAS TOTALES MAS PROX A LA REALIDAD
N=zeros(1,P);
for p=1:P
    n=0;
    for cc=1:CantDeTirasTotales
        if length(find((Datos(:,p))==cc))
            n=n+1;
        end
    end
    N(p)=n;
end
% CantDeTirasEstimada=min(N); 
CantDeTirasEstimada=max(N);

%FIN DEL LOOP DE PREPROCESAMIENTO Y RECALCULAMOS LA POBLACION INICIAL

% Generamos la matriz "Datos" de [CantDeTirasEstimada x P]
CantDeTirasAEmplear=CantDeTirasEstimada;
Datos=randint(CantDeTirasAEmplear,P,[0,CantDeTirasAEmplear]);     %generamos la poblacion inicial de soluciones, de manera RANDOM (Aleatoria)
Datos=Datos.*randsrc(CantDeTirasAEmplear,P,[0,1;0.2,0.8]);      %agregamos ceros
...
% Coherencia=(LargosTot-SumValor).^4;
Aptitud=CantidadDeTirasEmpleadas+Desp;       %aptitud del conjunto de soluciones
...
UmbralApareo=0.98*AptiProm;              %el umbral de apareo debe ser menor que la media, ya que menor aptitud es mejor
ProbaMuta=randsrc(1,1,[0 1;.99 .01]);   %debemos ser cautelosos con la pobabilidad de Mutacion < 1/100
...
clear i j;  %Liberamos de la memoria las variables "j" e "i" utilizada anteriormente, esto se deberia haber realizado 
...
j=min(Aptitud);
i=find(Aptitud==j);                  %indice de la mejor solucion de la ultima poblacion (menor aptitud numerica)
